...
async function validateTeam(playerIds, captainId, viceCaptainId, match) {
  ...
  if (batTotal < 3) return 'Team must have at least 3 Batters (WK included)';
  if (roleCounts.AR < 1) return 'Team must have at least 1 All-Rounder';
  if (roleCounts.BOWL < 2) return 'Team must have at least 2 Bowlers';
...
const upsertTeam = async (req, res) => {
  ...
  try {
    ...
    // Check if team is locked (scores already submitted) — must check before deadline logic
    ...
    // Exception: auto-generated teams can be edited after deadline until scores are locked
    if (new Date() >= match.deadline) {
      if (!existingTeam?.isAutoGenerated) {
        return res.status(403).json({ message: 'Team submission deadline has passed' });
      }
      // Auto-generated team being edited after deadline — allowed, clear the flag
    }
