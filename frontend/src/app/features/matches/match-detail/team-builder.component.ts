              ...
              <div class="flex items-center gap-2">
                ...
                @if (isAutoGenerated()) {
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium"
                    ...
                    <mat-icon style="font-size: 14px; width: 14px; height: 14px;">casino</mat-icon>
                    Auto-picked
                  </span>
                }
              </div>
              @if (!isDeadlinePassed() || canEditAutoTeam()) {
...
export class TeamBuilderComponent implements OnInit {
  ...
  readonly isAutoGenerated = signal<boolean>(false);
  ...
  // Auto-generated teams can be edited after deadline (until scores are locked)
  readonly canEditAutoTeam = computed(() => this.isAutoGenerated() && this.isDeadlinePassed());

  readonly canSubmit = computed(() => this.validationError() === '' && (!this.isDeadlinePassed() || this.canEditAutoTeam()));
  ...
  ngOnInit() {
    this.api.getMyTeam(this.matchId()).subscribe({
      next: (team) => {
        ...
        this.isAutoGenerated.set(!!(team as any).isAutoGenerated);
  ...
  togglePlayer(player: Player) {
    if (this.isDeadlinePassed() && !this.canEditAutoTeam()) return;
